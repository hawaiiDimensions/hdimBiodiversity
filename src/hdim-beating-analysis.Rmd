...
# calculate spearmans coorelation coef
```{r}
# cor.test(x, y, method="spearman")
# x, y numeric vectors with same length
# check for correlation bw num zotus, num plants, log flow age, mntd, inv simp

# number of zotus per plot
num.zotus.plot <- endemic %>%
  group_by(sitePlot, site, flowAge, volcano) %>% 
  summarise(n_otu = n_distinct(otu))
num.zotus.plot

# number of plants per plot
uniquePlant.plot <- endemic %>%
  group_by(sitePlot, site, flowAge, volcano) %>%
  summarise(n_plant_sampled = n_distinct(smplUnit)) 
uniquePlant.plot

# corr of plants and zotus
cor.test(uniquePlant.plot$n_plant_sampled, num.zotus.plot$n_otu, method = "spearman")
# rho = 0.6193716 < .7

plot(uniquePlant.plot$n_plant_sampled, num.zotus.plot$n_otu)

# corr of log sub age and zotus
cor.test(log(num.zotus.plot$flowAge), num.zotus.plot$n_otu, method = "spearman")
# 0.6872662 < .7

plot(log(num.zotus.plot$flowAge), num.zotus.plot$n_otu)

# corr of log sub age and plants
cor.test(log(uniquePlant.plot$flowAge), 
         uniquePlant.plot$n_plant_sampled, 
         method = "spearman")
# 0.6479429 < .7

plot(log(uniquePlant.plot$flowAge), 
         uniquePlant.plot$n_plant_sampled)
```

# plot inv simpson and num zotus at plot level with time

```{r}
############# calculate inverse Simpson at plot level ##########
# first need matrix at plot level
PlotAbun <- endemic %>% 
    select(otu, abundance, site, sitePlot, smplUnit) %>%
    group_by(sitePlot, otu) 
...
# check plot data in same order
# levels(PlotAbun$sitePlot) == levels(plotDF$sitePlot)

# Join to plot data objects from previous code block
plotDF <- num.zotus.plot %>%
  left_join(uniquePlant.plot)

SimpPlot <- cbind(plotDF, invSimpPlot)
colnames(SimpPlot)[7] <- "invSimp"
SimpPlot

SimpPlot %>%
  pivot_longer(cols = c("n_otu",
                        "n_plant_sampled",
                        "invSimp"), 
               names_to = "richness") %>%
  ggplot(aes(x = log(flowAge), y = value, fill = richness)) +
  geom_point(pch = 21) +
  facet_wrap(~richness, scales = "free") +
  theme(legend.position = "none")

ggsave("data/github/invSimp_otu_plnt_time_jitter.png", width = 6, height = 4, units = "in")  

# corr of log sub age and inv simp
cor.test(log(SimpPlot$flowAge), 
         SimpPlot$invSimp, 
         method = "spearman")
# 0.6856335  < .7

plot(log(SimpPlot$flowAge), 
         SimpPlot$invSimp)
```

# Species accumulation curves to det which orders have > 80% and should be included in SEMS
# Doing this for endemic taxa only since these are the focus of our biodiversity over time study
```{r, message = FALSE, warning = FALSE}
# first create a matrix of plots (rows) by zotus (columns) matrix
# need to create one for each order so write a function to filter by order and use lapply

unique(endemic$orderGrp)

orderCoverage <- function(o){
orderMtrx <- endemic %>%
  select(orderGrp, sitePlot, otu, abundance) %>% # select features (makes it easier to see what you are working with)
  filter(orderGrp == o) %>% # filter by order
  group_by(sitePlot, otu) %>%
  summarise(abundance = sum(abundance)) %>% # sum abundances for each otu that were sampled on various plants
  pivot_wider(names_from = otu, values_from = abundance)
orderMtrx
...
orderMtrx <- orderMtrx %>%
  mutate_if(is.numeric, replace_na, replace = 0)
orderMtrx <- orderMtrx[,-1]
orderMtrx <- as.matrix(as.data.frame(orderMtrx))

# Function specaccum (vegan package) finds species accumulation curves or the number of species for a certain number of sampled sites or individuals.
plot(specaccum(orderMtrx, method = "random"), main = o)

}

#orderCoverage("Orthoptera")

lapply(unique(endemic$orderGrp), orderCoverage)

```

# use iNEXT to evaluate coverage 
```{r, message = FALSE}
# convert to species by sites
inextCoverage <- function(o){
orderMtrx <- endemic %>%
  select(orderGrp, site, otu, abundance) %>% # select features (makes it easier to see what you are working with)
  filter(orderGrp == o) %>% # filter by order
  group_by(site, otu) %>%
  summarise(abundance = sum(abundance)) %>% # sum abundances for each otu that were sampled on various plants
...
orderMtrx <- orderMtrx %>%
  mutate_if(is.numeric, replace_na, replace = 0)
orderMtrx <- as.data.frame(orderMtrx)
orderMtrx <- column_to_rownames(orderMtrx, "otu")

# create iNEXT object
otuSite <- iNEXT(orderMtrx, q = 0, datatype = "abundance")

# use ggiNEXT to plot
ggiNEXT(otuSite, type = 1, se = TRUE, facet.var = "none",
   color.var = "site", grey = FALSE)

# estimate coverage
#cbind(o, (estimateD(orderMtrx, datatype="abundance", base="coverage", level=0.80, conf=0.95)))
}

#inextCoverage("Orthoptera")
lapply(unique(endemic$orderGrp), inextCoverage)
